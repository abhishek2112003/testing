...
class ExportMenu extends GridView {
     ...
     */
    public $dropdownOptions = ['class' => 'btn btn-default exp_button'];
    ...
    protected function initColumnSelector() {
        ...
        $selector = [];
        Html::addCssClass($this->columnSelectorOptions, 'btn btn-default exp_button dropdown-toggle');
    ...
    public function generateRow($model, $key, $index) {
        ...
        foreach ($this->getVisibleColumns() as $column) {
            ...
            $cell = $this->_objPHPExcelSheet->setCellValue(
                    self::columnName($this->_endCol) . ($index + $this->_beginRow + 1), strip_tags($value), true
            );
